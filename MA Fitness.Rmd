...
ggplot() + geom_point(aes(x=x, y=slopes2), color='red') +
  geom_point(aes(x=x[2:(length(x)-1)], y=slopes), alpha=0.3)
...
d %>%
  filter(date == mdy(11022023)) %>% ggplot() +
...
assay.data %>% ggplot() +
  ...
  geom_line(data = t, mapping=aes(x=x, y=y), color='Red', linewidth=1)
