...
- this fits a linear regression model and extracts the sum of squared residuals
```{r}
well.lm.sse <- function(x, y){
  sum((summary(lm(y~x))$residuals)^2)
}
```
...
dfs <- lapply(l[endsWith(l, '.csv') & startsWith(l, 'Ref ')], function(r){
  d <- read.csv(r)
  ...
  d$sse <- sapply(c(1:nrow(d)), function(r){
    well.lm.sse(assay.data$Time, assay.data[,which(names(assay.data)==d$well[r])])
  })
...
Adding additional information for categorization. Exluding MA lines that changed ploidy. Excluding batch 1 due to lack of diploid controls.
```{r}
...
d %>%
  filter(category == 'Ctrl.H') %>% ggplot() + geom_point(aes(x=treatment, y=slope, color=treatment)) + facet_wrap(vars(date))
```
...
```{r}
# df2 %>% ggplot() + geom_point(aes(x=category, y=slope, color=category)) + facet_wrap(vars(date))
```
...
```{r}
# df <- df %>% filter(lm.slope >= 0)
# df2 <- df2 %>% filter(lm.slope >= 0)
```

```{r}
# diff <- anti_join(d, df, by=join_by(well, date))
# diff %>% select(well, date) %>% count(date)
# diff %>% filter(date == ymd(20230503)) %>% 
#   select(well) %>% as.vector %>% print()

# 
# diff <- anti_join(d, df2, by=join_by(well, date))
# diff %>% select(well, date) %>% count(date)
# diff %>% filter(date == ymd(20230503)) %>% 
#   select(well) %>% as.vector %>% print()
```
...
Plotting the time stamp for max slope
```{r}
# df %>% ggplot() + geom_point(aes(x=category, y=time, color=category)) + facet_wrap(vars(date))
```

```{r}
# df %>% filter(!(category == 'Blank')) %>% filter(time<10) %>% ggplot() + geom_histogram(aes(x=time), fill='black')
# 
# df %>% filter(time<1) %>% 
#   filter(date == ymd(20230503)) %>% select(well) %>%
#   as.vector()
```

Plotting to explore the linear models
```{r}
# df2 %>% filter(!(category == 'Blank')) %>% ggplot() + geom_histogram(aes(x=lm.slope), fill='black')
# 
# df2 %>% filter(!(category == 'Blank')) %>% ggplot() + geom_histogram(aes(x=sse), fill='black')
```

Plotting for more factors
```{r}
# df$delta = df$final - df$initial
# df %>%
#   ggplot() + 
#   geom_histogram(aes(x=initial), fill = "red", alpha = 0.5) +
#   geom_vline(xintercept = mean(good.blanks$initial))
#   # geom_histogram(aes(x=final), fill = "blue", alpha = 0.5) + 
#   # geom_histogram(aes(x=delta), fill = "green", alpha = 0.5) 
# 
# 
# #if the OD decreases at the end, remove the replicates
# df %>% filter(delta < 0) %>% 
#   filter(date == ymd(20230503)) %>% select(well) %>%
#   as.vector()
```
...
Data Analysis
```{r}
data <- data %>% select(date, well, treatment, label, category, slope) %>%
...
```{r}
data %>% ggplot() + geom_point(aes(x=treatment, y=slope, color=treatment))
```
...
lmer for hypothesis testing
```{r}
null <- lmer(slope ~ MA + ploidy + (1|date) + (1|treatment), data2)
summary(null)
full <- lmer(slope ~ MA*ploidy + (1|date) + (1|treatment), data2)
summary(full)
...
full <- lmer(slope ~ ploidy * MA + initial + (1|treatment) + (1|date), data2)
summary(full)
null <- lmer(slope ~ ploidy + MA + initial + (1|treatment) + (1|date), data2)
anova(full, null)
...
Growth rate curve
```{r}
p <- read.csv("Ref 05 03.csv")

assay.name <- "Rutuja 05 03.txt"
assay.data <- read.delim(assay.name)
...
assay.data$Time<-seq(from=0.25,by=0.25,length.out=nrow(assay.data))
# calculate slopes
p$slope <- sapply(c(1:nrow(p)), function(r){
  # spline.slope(assay.data$Time, assay.data[,which(names(assay.data)==p$well[r])])
  max(nderiv(loess(log(assay.data[,which(names(assay.data)==p$well[r])]) ~ assay.data$Time, degree=1, span=0.2), seq(min(assay.data$Time), max(assay.data$Time), length=101)), na.rm=TRUE)
})


fun <- function(r){
  # spline.slope(assay.data$Time, assay.data[,which(names(assay.data)==p$well[r])])
  (nderiv(loess(log(assay.data[,which(names(assay.data)==p$well[r])]) ~ assay.data$Time, degree=1, span=0.2), seq(min(assay.data$Time), max(assay.data$Time), length=101)))
}


p$time <- sapply(c(1:nrow(p)), function(r){
    spline.time(assay.data$Time, assay.data[,which(names(assay.data)==p$well[r])])
  })

#nderiv(loess(assay.data$M15 ~ assay.data$Time, degree=1, span=0.2), assay.data$Time)
#max(nderiv(loess(assay.data$M15 ~ assay.data$Time, degree=1, span=0.2), assay.data$Time), na.rm=TRUE)
#which.max(nderiv(loess(assay.data$M15 ~ assay.data$Time, degree=1, span=0.2), assay.data$Time))
#assay.data$M15[44]

t <- data.frame(x = seq(10, 12, 0.25))
slope = 0.05546986
intercept = 0.442 - 0.05546986 * 11
t <- t %>% mutate(y = slope * x + intercept)

assay.data %>% ggplot() +
  geom_line(aes(x=Time, y=M15)) +
  scale_x_continuous(limits = c(0,24)) +
  ylab("Optical Density") + 
  geom_line(data = t, mapping=aes(x=x, y=y), color='Red')
