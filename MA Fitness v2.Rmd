...
ess <- loess(log(y) ~ x, degree=1, span=0.05)
p <- predict(ess, x)
```
...
```{r}
null <- lmer(slope ~ MA*ploidy + (1|lineid), df)
summary(null)
full <- lmer(slope ~ MA*ploidy + (1|day) + (1|lineid), df)
summary(full)
...
Some haploids have a relative fitness that is greater than 0. They have the benefit of recessive beneficial mutations which are not masked by a second set of chromosomes. No beneficial effects observed for diploids.
...
```{r}
null <- lmer(slope ~ ploidy + (1|day) + (1|lineid), df)
full <- lmer(slope ~ mutations + ploidy + (1|day) + (1|lineid), df)
summary(full)
